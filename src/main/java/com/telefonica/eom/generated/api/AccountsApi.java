...
public interface AccountsApi {
    ...
    @ApiOperation(value = "Create new account product instance", notes = "", response = ProductInstanceType.class, tags={ "products of an account", })
    @ApiResponses(value = { 
    ...
    @RequestMapping(value = "/accounts/{accountId}/products",
        produces = { "application/json" }, 
        consumes = { "application/json" },
        method = RequestMethod.POST)
    ResponseEntity<ProductInstanceType> createAccountProductInstance(@ApiParam(value = "ID of the account the products will be attached to",required=true ) @PathVariable("accountId") String accountId,
    ...
    @ApiOperation(value = "Retrieve list of products in inventory associated to a given account", notes = "", response = ProductInstanceType.class, responseContainer = "List", tags={ "products of an account", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "List of product instances provided", response = ProductInstanceType.class) })
    @RequestMapping(value = "/accounts/{accountId}/products",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<List<ProductInstanceType>> retrieveAccountProductInstances(@ApiParam(value = "ID of the account whose products need to be fetched",required=true ) @PathVariable("accountId") String accountId,
        @ApiParam(value = "If this API is used via a platform acting as a common entry point to different OBs, this identifier is used to route the request to the corresponding OB environment"  ) @RequestHeader(value="UNICA-ServiceId", required=false) String unICAServiceId,
        @ApiParam(value = "Identifier for the system originating the request"  ) @RequestHeader(value="UNICA-Application", required=false) String unICAApplication,
        @ApiParam(value = "Including the proof of access (using OAuth2.0 security model) to guarantee that the consumer has privileges to access the entity database"  ) @RequestHeader(value="Authorization", required=false) String authorization,
         ...
         @ApiParam(value = "To obtain the list of products matching to a given legal identification type (e.g.: DNI, passport, social security number, RUC, CPF, driver's license, etc.). This is typically combined with nationalID to query for specific identity") @RequestParam(value = "nationalIdType", required = false) String nationalIdType,
